...
def collect_bounding_box(anno_path, out_filename):
    ...
    for f in glob.glob(os.path.join(anno_path, "*.txt")):
        ...
        ins_bbox_label = np.expand_dims(np.concatenate([xyz_min, xyz_max, np.array([label])], 0), 0)
...
def bbox_label_to_obj_room(
    input_filename, out_filename_prefix, easy_view=False, permute=None, center=False, exclude_table=False
):
    """Visualization of bounding boxes.
...
def collect_point_bounding_box(anno_path, out_filename, file_format):
    ...
    for f in glob.glob(os.path.join(anno_path, "*.txt")):
        ...
        point_bbox_list.append(np.concatenate([points, labels, xyz_offsets, dimensions], 1))  # Nx13
